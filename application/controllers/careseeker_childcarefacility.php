...
class Careseeker_childcarefacility extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_childcarefacility_model','childcarefacility');
        ...
        $limit = $this->input->get('limit');
               ...
               $sess_data = array(
                ...
                $this->session->set_userdata($sess_data);
                   ...
                   if($limit){
                            if($care == 'care'){
                               $users            = $this->user_model->orderByCare($care_id,$latitude,$longitude);
                           }else{
    ...
    function getPages(){
        ...
        if($this->input->is_ajax_request()){
            ...
            $total_rows = $this->childcarefacility->count($lat,$lon);
    ...
    function pages(){
        ...
        $this->breadcrumbs->push('childcarefacility', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
       if(check_user()){
            $locationdetails = $this->common_model->getMyLocation(check_user());
                if($locationdetails){
        ...
        $ipdata     = $this->common_model->getIPData($this->ipaddress);
        $userdata   = $this->childcarefacility->getAllchildcarefacility($latitude,$longitude,$per_page,$offset);
        
        $data['main_content']  = 'frontend/careseeker_childcarefacility/index';
        $data['userdatas']     = $userdata;
        $data['userlogs']      = $this->user_model->getUserLog();
        $data['countries']     = $this->common_model->getCountries();
        $data['ipdata']        = $ipdata;
        $data['title']         = 'Child Care Facility';
    ...
    public function search(){
		$limit = 15;		
		  if(check_user()){
            ...
            $res = $this->childcarefacility->search($postdata,$latitude,$longitude);
			if(is_array($res))
				$total = count($res);
			else
				$total = 0;
        ...
        public function searchbylocation(){
            if($_GET){
                ...
                $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                $offset =    ($page - 1) * $per_page;
                $url = site_url().'careseeker_childcarefacility/index/';
                
                $users   = $this->childcarefacility->searchbylocation($latitude,$longitude);
                if($users != false)
