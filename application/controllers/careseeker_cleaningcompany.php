...
class Careseeker_cleaningcompany extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_cleaningcompany_model','cleaningcompany');
	...
	public function index(){
        ...
        $option = "distance";
        $account_category = 3;
        $care_type = 28;//blank for careseekers and caregiver
        if(segment(1) == 'caregivers') $title = "Workers for cleaning company";
        else $title = "Workers for cleaning company jobs";        
        $distance = "unlimited";                     
    ...
    function pages(){
        $this->breadcrumbs->push('cleaningcompany', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
       if(check_user()){
            $locationdetails = $this->common_model->getMyLocation(check_user());
                if($locationdetails){
        ...
        $ipdata     = $this->common_model->getIPData($this->ipaddress);
        $userdata   = $this->cleaningcompany->getAllcleaningcompany($latitude,$longitude,$per_page,$offset);
        
        $data['main_content']  = 'frontend/careseeker_cleaningcompany/index';
    ...
    public function search(){
        ...
        if($_GET){
            ...
            $page = $this->uri->segment(3)?$this->uri->segment(3):1;
            $offset =    ($page - 1) * $per_page;
            $url = site_url().'careseeker_cleaningcompany/index/';
            
            $users   = $this->cleaningcompany->searchbylocation($latitude,$longitude);
            if($users != false)
