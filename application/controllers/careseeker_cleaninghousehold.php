...
class Careseeker_cleaninghousehold extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_cleaninghousehold_model','cleaninghousehold');
	...
	public function index(){
        ...
        $option = "distance";
        $account_category = 2;
        $care_type = 24; //blank for careseekers and caregiver
        $title = "Cleaning / household help jobs";        
        $distance = "unlimited";                     
    ...
    function pages(){
        $this->breadcrumbs->push('cleaninghousehold', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
       ...
       $userdata   = $this->cleaninghousehold->getAllcleaninghousehold($latitude,$longitude,$per_page,$offset);
        
        $data['main_content']  = 'frontend/careseeker_cleaninghousehold/index';
    ...
    public function search(){
		...
		if (!$latitude || !$longitude || !$location) {		
		  if(check_user()){
            ...
            $result = $this->cleaninghousehold->search($postdata,$latitude,$longitude);
			if(is_array($result))
				$total = count($result);
			else
				$total = 0;
        ...
        public function searchbylocation(){
            if($_GET){
                ...
                $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                $offset =    ($page - 1) * $per_page;
                $url = site_url().'careseeker_cleaninghousehold/index/';
