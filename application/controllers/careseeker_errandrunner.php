...
class Careseeker_errandrunner extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_errandrunner_model','errandrunner');
    ...
    function getPages(){
        ...
        if($this->input->is_ajax_request()){
            ...
            $total_rows = $this->errandrunner->count($latitude,$longitude);
    ...
    function pages(){
        ...
        $this->breadcrumbs->push('errandrunner', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
        ...
        $userdata   = $this->errandrunner->getAllerrandrunner($latitude,$longitude,$per_page,$offset);
        
        $data['main_content']  = 'frontend/careseeker_errandrunner/index';
    ...
    public function search(){
		...
		if (!$latitude || !$longitude || !$location) {		
            ...
            $result = $this->errandrunner->search($postdata,$latitude,$longitude);
			if(is_array($result))
				$total = count($result);
			else
				$total = 0;
            ...
            public function searchbylocation(){
                if($_GET){
                    ...
                    $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                    $offset =    ($page - 1) * $per_page;
                    $url = site_url().'careseeker_errandrunner/index/';
