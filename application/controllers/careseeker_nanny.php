...
class Careseeker_nanny extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_nanny_model','nanny');
	...
	public function index(){
        ...
        $option = "distance";
        $account_category = 2;
        $care_type = 18;//blank for careseekers and caregiver
        $title = "Nanny jobs";        
        $distance = "unlimited";                     
    ...
    function getPages(){
        if($this->input->is_ajax_request()){
            ...
            $total_rows = $this->nanny->count_nanny();
    ...
    function pages(){
        ...
        $this->breadcrumbs->push('Nanny', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
        ...
        $data['main_content']  = 'frontend/careseeker_nanny/index';
		...
		if (!$latitude || !$longitude || !$location) {		
		  if(check_user()){
            ...
            $result = $this->nanny->search($postdata,$latitude,$longitude);
			if(is_array($result))
				$total = count($result);
			else
				$total = 0;
            ...
            public function searchbylocation(){
                if($_GET){
                    ...
                    $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                    $offset =    ($page - 1) * $per_page;
                    $url = site_url().'careseeker_nanny/index/';
                    
                    $users   = $this->nanny->searchbylocation($latitude,$longitude);
                    if($users != false)
