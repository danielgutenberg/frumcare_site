...
class Careseeker_seniorcarefacility extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
		$this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_seniorcarefacility_model','seniorcarefacility');
    ...
    function pages(){
        $this->breadcrumbs->push('seniorcarefacility', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
       if(check_user()){
            $locationdetails = $this->common_model->getMyLocation(check_user());
                if($locationdetails){
        ...
        $ipdata     = $this->common_model->getIPData($this->ipaddress);
        $userdata   = $this->seniorcarefacility->getAllseniorcarefacility($latitude,$longitude,$per_page,$offset);
    ...
    public function search(){
		...
		if (!$latitude || !$longitude || !$location) {	
		  if(check_user()){
            ...
            $result = $this->seniorcarefacility->search($postdata,$latitude,$longitude);
			if(is_array($result))
				$total = count($result);
			else
				$total = 0;
            ...
            public function searchbylocation(){
                if($_GET){
                    ...
                    $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                    $offset =    ($page - 1) * $per_page;
                    $url = site_url().'careseeker_seniorcarefacility/index/';
                    
                    $users   = $this->seniorcarefacility->searchbylocation($latitude,$longitude);
                    if(is_array($users))
