...
class Careseeker_specialneedsfacility extends CI_Controller{
	public function __construct(){
		parent:: __construct();
		$this->load->library('breadcrumbs');
        $this->ipaddress = $_SERVER['REMOTE_ADDR'];
		$this->load->model('careseeker_specialneedsfacility_model','specialneedsfacility');
	...
	public function index(){
        ...
        $option = "distance";
        $account_category = 3;
        $care_type = 27; //blank for careseekers and caregiver
        if(segment(1) == 'caregivers') $title = "Workers / staff for special needs facility";
        else $title = "Workers / staff for special needs facility jobs";        
        $distance = "unlimited";                     
    ...
    function getPages(){
        if($this->input->is_ajax_request()){
            ...
            $total_rows = $this->specialneedsfacility->count($lat,$lon);
            $number_pages = ceil($total_rows/$per_page);
            echo $number_pages;
        }
    }
    function pages(){
        $this->breadcrumbs->push('specialneedsfacility', site_url().'#');
        $this->breadcrumbs->unshift('Home', base_url());
       if(check_user()){
            $locationdetails = $this->common_model->getMyLocation(check_user());
                if($locationdetails){
        ...
        $ipdata     = $this->common_model->getIPData($this->ipaddress);
        $userdata   = $this->specialneedsfacility->getAllspecialneedsfacility($latitude,$longitude,$per_page,$offset);
        
        $data['main_content']  = 'frontend/careseeker_specialneedsfacility/index';
        $data['userdatas']     = $userdata;
        $data['userlogs']      = $this->user_model->getUserLog();
        $data['countries']     = $this->common_model->getCountries();
        $data['ipdata']        = $ipdata;
        $data['care_type']     = $this->caretype_model->getCare(2,2);
        $data['title']         = 'Special Needs Facility';
        $this->load->view(FRONTEND_TEMPLATE, $data);
    ...
    public function search(){
		...
		if(check_user()){
            ...
            $result = $this->specialneedsfacility->search($postdata,$latitude,$longitude);
			if(is_array($result))
				$total = count($result);
			else
				$total = 0;
            ...
            public function searchbylocation(){
                if($_GET){
                    ...
                    $page = $this->uri->segment(3)?$this->uri->segment(3):1;
                    $offset =    ($page - 1) * $per_page;
                    $url = site_url().'careseeker_specialneedsfacility/index/';
                    
                    $users   = $this->specialneedsfacility->searchbylocation($latitude,$longitude);
                    if($users!= false)
