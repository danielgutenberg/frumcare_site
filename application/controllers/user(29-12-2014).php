...
class User extends CI_Controller
{
    ...
    public function notifications(){
        ...
        $config['base_url'] = base_url()."user/ticket_view";
     ...
     //changes by kiran
      ...
      public function go_to_profile(){
        ...
        redirect('caregivers/details/'.$uri);
      }
      ...
      public function reviews(){
        ...
        $this->load->library('pagination');
        $config['base_url'] = base_url()."user/myreview/";
        ...
        if($acc_details->account_category == '1'){
            $field = "profile_id";
        }

        if($ac c_details->account_category == '2'){
            $field = "user_id";
        } */
	...
	//by kiran
      ...
      public function addprofile(){
		...
		if($account_details->account_category == '1'){
	            @$title = "Add New Profile";
	        }
	
	        if($account_details->account_category == '2'){
	            @$title = "Add New Job";
	        }
	        
	        if($account_details->account_category == '3'){
	            @$title = "Add New Job";
	        }
	}
      ...
      public function new_profile(){
            if(isset($_GET['value'])){
                ...
                $exp = explode('_', $_GET['value']);
                $submit_id = $exp[0];
                $acc_details = get_account_details();
                $account_type = $exp[1];
                $data['record'] = array('submit_id' => $submit_id,'ac_type' => $ac_type,'account_type' => $account_type);
                if($acc_details->account_category == 1 ){
                    if($submit_id == 1){
                        $profile_check = $this->user_model->existing_profile_check($acc_details->account_category,$submit_id);
                        if($profile_check){
                            $this->load->view('frontend/care/giver/babysitter_form',$data);
                        }
                        else{
                            echo "You have already own this profile";
                        }
                    }
                    else if($submit_id == 2){
                        $profile_check = $this->user_model->existing_profile_check($acc_details->account_category,$submit_id);
                     ...
                     //   $this->load->view('frontend/care/giver/nanny_form',$data);
                      ...
                      //  $this->load->view('frontend/care/giver/nanny_form');
                        ...
                        if($profile_check){
                          ...
                          $profile_check = $this->user_model->existing_profile_check($acc_details->account_category,$submit_id);
                        if($profile_check){
                            $this->load->view('frontend/care/giver/specialneedscenter_form');
                        }
                        else{
                            echo "You have already own this profile";
                        }                    
                        ...
                        if($profile_check){
                          ...
                          $profile_check = $this->user_model->existing_profile_check($acc_details->account_category,$submit_id);
                        if($profile_check){
                            $this->load->view('frontend/care/seeker/seniorcarecenter_form',$data);
                        }
                        else{
                            echo "You have already own this profile";
                        }                        
                        ...
                        echo "You are not allowed to become Careseeker";
                }
                else{
                     if($submit_id == 17){
                          $profile_check = $this->user_model->existing_profile_check($acc_details->account_category,$submit_id);
  ...
  /*  public function addprofileconfirm(){
            ...
            if($profile_check){
                ...
                $insert = array(
                    ...
                    'age_group' => isset($p['age_group']) ? $p['age_group'] : '',
                    ...
                    'rate' => isset($p['rate']) && isset($p['rate_type']) ?$p['rate']:'',
      ...
      */
      ...
      public function createsearchalert(){
        ...
        $this->breadcrumbs->unshift('My Search Alert', base_url().'user/searches');
        $this->breadcrumbs->unshift('My Account', base_url().'user/dashboard');
      ...
      public function searchalerts(){
        $current_user   = $this->session->userdata['current_user'];
        $myalert        = $this->user_model->getSearchAlertByUserId($current_user);
        ...
        $data = array(
                ...
                'record'        => $this->user_model->getAlertData($myalert)
           );
      ...
      public function deleteticket($id = ''){
        ...
        redirect('user/notifications','refresh');
      ...
      // added on 22 2014
       ...
       public function pay(){
        ...
        $data['main_content'] = 'frontend/payment/pay';
        $this->load->view(FRONTEND_TEMPLATE,$data);
     ...
     public function membership(){
      ...
      public function account($id_hash = '' ){
            if(isset($_POST['save'])){
			 ...
			 $q = $this->common_model->update('tbl_user',$insert1,array('id'=>check_user()));
                             if($q){
                                $this->common_model->update('tbl_userprofile',$insert2,array('user_id'=>check_user()));        
                        	redirect('user/details/'.$id_hash,'refresh');         
                             }
