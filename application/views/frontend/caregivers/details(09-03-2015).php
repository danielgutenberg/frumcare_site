		...
		<div class="left-sidebar-details">
            			...
            			if($this->session->userdata('current_user')!=$care_id){ //condition for blocking own review and rating
            				...
            				<?php }
                                ...
                                if($recordData['care_type']<17){                                    
                                    ?>
                                    <a href="<?php echo site_url()?>login">Write a review</a>                                
                                    <?php
                                 }                                                          
                            }
            			}?>
            ...
            if($this->uri->segment(4)>0 && $this->uri->segment(4)<10){
                ...
                if(!empty($recordData['rate'])){ ?>                                                        
                    <?php $rate_type = $recordData['rate_type']==2?'Per Month':'Per Hour'?>
                    <span class="hour-wrap">$<?php echo $recordData['rate'].'<span>'.$rate_type.'</span>'; ?></span>
                    <?php
                }
                else{ ?>
                ...
                if(!empty($recordData['experience'])){ ?>
                    ...
                    <span class="experience-wrap"><?php 'N/A'. ' <span>Years Experience</span>';?></span>
                    <?php   
                }                
                ?>                             	        			    			
                ...
                if(!empty($recordData['location'])){ ?>
                    ...
                    <?php $formated_location = $location_array[0].', '.$location_array[1]; ?> 
                ...
                <?php                
                    ...
                    if(in_array('immediate',$availablility)){?>
                        <span class="location-wrap"><?php echo 'Immediate'.'<span>Availability</span>';?></span>
                        <?php    
                    }
            ...
            if($this->uri->segment(4)>16 && $this->uri->segment(4)<25){                 
                if(!empty($recordData['location'])){ ?>
                    <?php $location_array = explode(',',$recordData['location']); ?>
                    <?php $formated_location = $location_array[0].', '.$location_array[1]; ?>
                ...
                <span class="hour-wrap">
    				<?php 
    				echo ucwords($type[0]['service_name']).'<span>Job Type</span>';
    				?>
    			</span>
                <?php
                if(!empty($recordData['rate'])){ ?>
                    <?php $rate_type = $recordData['rate_type']==2?'Per Month':'Per Hour'?>
                    <span class="experience-wrap">$<?php echo $recordData['rate'].'<span>'.$rate_type.'</span>'; ?></span>
                    <?php
                }                                
                else{ ?>
            ...
            if(($this->uri->segment(4)>9 && $this->uri->segment(4)<17) || ($this->uri->segment(4)>24 && $this->uri->segment(4)<29)){
                if(!empty($recordData['location'])){ ?>
                ...
                if(!empty($recordData['rate'])){ ?>
                    <span class="experience-wrap">$<?php echo str_replace("t","-",$recordData['rate']).'<span>'.$recordData['rate_type']==2?'Per Month':'Per Hour'.'</span>'; ?></span>
                    <?php
                }                                
                else{ ?>
		...
		<div class="meet-caregivers-clients">
            ...
            ?>
             <div class="similiar-care">             
						<h2>
                        <?php if($recordData['care_type'] > 16 && $recordData['care_type'] < 25){
                                    echo "Similar Jobs";
                                }
                                else{
						...
						<?php                       
							if($similar_types!=''){
				                ...
				                $navigate = $caregiver['care_type']>16?'careseekers':'caregivers'; ?>					 							
						...
						<div class="similar-care-providers care-providers1">
							...
							<div class="care-hours">
								<?php if($caregiver['rate']!=''){
                                        ...
                                        echo '$'.str_replace('t', '- $', $caregiver['rate']).$type;
								}else{
										echo '$0/hour';
									}?>
							</div>
                ...
                <div class="verification-sidebar">
                    ...
                    <ul class="check-training">
                        ...
                        <li class="bbc">Other Training</li> 
                            ...
                            <div class="share-profile">                                    
                                    <div class="texts">Save this profile to your list of favorites in your Frumcare account?</div>
                                </div>
                            <div class="share-profile-wrap">
