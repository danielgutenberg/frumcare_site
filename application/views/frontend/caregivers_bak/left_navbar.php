<?php echo $this->breadcrumbs->show();?>
	...
	if(segment(1) == 'therapists'){
	 ...
	 if(segment(1) == 'caregivers'){
            ...
            echo '<h3>Caregivers</h3>';
	 }
     
     if(segment(1) == 'careseekers'){
        echo '<h3>Careseekers</h3>';
     }
	 ?>
	  ...
	  <div class="left-search-panel">
	 	...
	 	<!-- 		<div class="citys">
	 			...
	 			<!-- <div class="neighborhood">
		 			...
		 			<div class="choose-services">
		 				<label>Choose Care Type</label>
		 				<select name="care_type" id="care_type">
                        <option value="">--select--</option>
                        <?php
                        if(segment(1) == 'caregivers'){
                            $care_type = getCare(1,1);
                        }else if(segment(1) == 'careseekers'){
                            $care_type = getCare(2,1);
                        }
                        
                        foreach($care_type as $care)
                        {
                            ?>
                                <option value="<?php echo $care['id'];?>"><?php echo $care['service_name']?></option>
                            <?php
                        }
                        ?>
	 	...
	 	<div>
	 		<!-- <form id="leftnavbar-down" method="post" action=""> -->
		 		...
		 		<div>
			 		<input type="checkbox" value="Imm" name="time[]" class="required">
			 		<span>Imm</span>
			 	</div>

				<div>
			 		<input type="checkbox" value="Start Date" name="time[]" class="required" id="ckbox1">
			 		<input type="text" name="start_date" id="textbox">
		 		...
		 		<div class="no-child">
		 		<span>Number of children requiring care</span>
		 		...
		 		<input type="checkbox" name="optional_number[]" value="Twins"> Twins 
		 		<input type="checkbox" name="optional_number[]" value="Triplets"> Triplets

		 		<div class="time-available" colspan="2"><h4>Abilities and skills(required)</h4>
