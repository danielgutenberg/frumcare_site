<div class="container">
 <?php $this->load->view('frontend/cleaninghousehold/left_navbar.php',$ipdata);?>
...
<?php if(is_array($ipdata)){?>
    ...
    <?php 
			...
			echo $count;
		?> </span>
	 Cleaing / Household help <span id="locationaddress"><?php echo $city;?></span></h3>
...
<script type="text/javascript">
		...
		$(document).ready(function(){
                ...
                window.location = '<?php echo site_url();?>'+locationaddress;
			});
			...
			var inital_sortlimit  = $('#sort').val();
      			if(inital_sortlimit){
      				$.ajax({
      					type:"get",
      					url:"<?php echo site_url();?>cleaningcompany/getPages",
      					...
      					success:function(data){
      						for (i= 1; i<=data; i++) {
                       			...
                       			$('.navigations').append('<a href="<?php echo site_url();?>cleaningcompany/pages/'+i+'"><span class="'+aclass+'">'+i+'</span></a>')
        ...
        $('#sort').change(function(){
  				...
  				$('.searchloader').fadeIn("fast");
  				  ...
  				  $.ajax({
  				  		type:"get",
  				  		url:"<?php echo site_url();?>careseeker_babysitter/sort_cate",
                          data:"limit="+per_page+"&order="+order+"&care=care&care_type="+care_type,
  				  		success:function(msg){
  							...
  							var pagenum = json.num;
  							var pagedata = json.userdatas;
                              var total_number = json.total_rows;
  							$('#list_container').html(pagedata);
  							...
  							var id = '<?php echo $this->uri->segment(3)?$this->uri->segment(3):1;?>';
  								for (i= 1; i<=pagenum; i++) {
  									if(i == id)
  	                  				...
  	                  				$('.navigations').append('<a href="<?php echo site_url();?>careseeker_babysitter/pages/'+i+'"><span class="'+aclass+'">'+i+'</span></a>')
  	            				}
  				  		 }
...
//            }
//        }
//        xmlhttp.open("GET","<?php echo base_url().'cleaningcompany/searchbylocation' ?>?latitude="+lat+"&longitude="+lng,true);
//        xmlhttp.send();
...
function myfunction(){
	...
	if((lat == '') && (lng == '')){
        ...
        xmlhttp.open("GET","<?php echo base_url().'cleaningcompany/searchbylocation' ?>?latitude="+lat+"&longitude="+lng+"&location="+location,true);
        xmlhttp.send();
	}
