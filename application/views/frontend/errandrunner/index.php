<div class="container">
 <?php $this->load->view('frontend/errandrunner/left_navbar',$ipdata);?>
        ...
        <span class="sort-by-relevance">
            ...
            <option>Sort By Caregivers</option>
                <?php
                    ...
                    {
                        ?><option  value="<?php echo $care['id']?>" <?php echo $care['id']==9 ?'selected':''?>><?php echo $care['service_name'];?></option><?php
                    }
                ?>			
			</select>
		</span>
		...
		$(document).ready(function(){
		  $('#sortby').change(function(){ 
			...
			if(inital_sortlimit){
				$.ajax({
					type:"get",
					url:"<?php echo site_url();?>errand_runner/getPages",
					...
					success:function(data){
						for (i= 1; i<=data; i++) {
                  			...
                  			$('.navigations').append('<a href="<?php echo site_url();?>errand_runner/pages/'+i+'"><span class="'+aclass+'">'+i+'</span></a>')
...
//            }
//        }
//        xmlhttp.open("GET","<?php echo base_url().'errand_runner/searchbylocation' ?>?latitude="+lat+"&longitude="+lng,true);
//        xmlhttp.send();
...
function myfunction(){
	...
	if((lat == '') && (lng == '')){
        ...
        xmlhttp.open("GET","<?php echo base_url().'errand_runner/searchbylocation' ?>?latitude="+lat+"&longitude="+lng+"&location="+location,true);
        xmlhttp.send();
	}
