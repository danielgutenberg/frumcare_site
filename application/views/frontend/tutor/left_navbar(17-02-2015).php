<?php echo $this->breadcrumbs->show();?>
	 	...
	 	<form method="post" id="left-nav" action="">
		 	...
		 	<div>
		 		...
		 		<div class="checkbox"><input type="checkbox" class="availability" value="By appointment">By appointment</div>
		 	</div>
...
<script>
	$(document).ready(function(){
		...
		$('.neighbour,.caregiverage_from,.caregiverage_to').blur(function(){
            ...
            var start_date = $("#textbox1").val()?$("#textbox1").val():'';
				$.ajax({
					...
					data:"neighbour="+neighbour+"&caregiverage_from="+caregiverage_from+"&caregiverage_to="+caregiverage_to+"&gender="+gender+"&language="+lang+"&observance="+observance+"&min_exp="+min_exp+"&availability="+availability+"&care_type="+care_type+"&subjects="+subjects+"&start_date="+start_date,
		...
		// onclick
		$('.gender,.lang,.observance,.availability,.subjects').click(function(){
			$(".searchloader").fadeIn("fast");
			var neighbour = $('.neighbour').val();
            var caregiverage_from = $('.caregiverage_from').val();
            var caregiverage_to = $('.caregiverage_to').val();
